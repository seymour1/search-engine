...
public class RetrievalEngine {
	...
	public static double dotProduct(ArrayList<ArrayList<PostingsInstance>> postings, int docNum){
		double total = 0;
		for(ArrayList<PostingsInstance> list : postings){
			for(PostingsInstance p : list){
				if(p.docNum==docNum)
					total += 1.0*p.termWeight;
			}
		}
		return total;
	}
	...
	public static void main(String[] args) {
		...
		if(args.length == 0){
			System.out.println("Incorrect number of arguments. Usage: retrieve search_terms");
			System.exit(-1);
		}
		String [] searchterms = args;
		...
		for(int i = 1; i < 504; i++){
			double dotProd = dotProduct(postings, i);
			if (dotProd > 0){
